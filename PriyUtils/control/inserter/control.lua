...
local migrations = require('control.inserter.migrations')

local calc = require('control.inserter.calc')
local floor = math.floor

local lib = require('control.inserter.lib')
...
local stopwatch = require('control.inserter.stopwatch')
...
local function populate_name_blacklist()
    if script.active_mods['miniloader'] then
        local inserters = game.entity_prototypes
        for name, prototype in pairs(inserters) do
            if prototype.type == 'inserter' and string.find(name, 'miniloader', nil, true) then
...
local function init_toggle_button(player)
    ...
    local toggle_element = top_gui['priyutils-inserter-throughput-toggle']
    if not toggle_element then
        toggle_element = top_gui.add{
            type = 'sprite-button',
            name = 'priyutils-inserter-throughput-toggle',
            tooltip = {'priyutils-inserter-throughput.toggle-button-tooltip'},
            sprite = 'priyutils-inserter-throughput-toggle-button',
            style = mod_button_style,
            auto_toggle = true,
        }
    end
    local player_settings = player.mod_settings
    toggle_element.visible = player_settings['priyutils-inserter-throughput-show-toggle'].value
    toggle_element.toggled = player_settings['priyutils-inserter-throughput-enabled'].value
end
...
local function on_entity_selected(event)
    ...
    if not player_settings['priyutils-inserter-throughput-enabled'].value then
        return
    end
    ...
    if current_entity then -- a valid inserter by now
        ...
        local precision = player_settings['priyutils-inserter-throughput-rounding-precision'].value
...
local function on_setting_changed(event)
    ...
    local player_index = event.player_index
    if setting_name == 'priyutils-inserter-throughput-show-toggle' then
        local player = game.get_player(player_index)
        local button = get_mod_button_flow(player)['priyutils-inserter-throughput-toggle']
        if button then
    ...
    elseif setting_name == 'priyutils-inserter-throughput-enabled' then
        local player = game.get_player(player_index)
        local button = get_mod_button_flow(player)['priyutils-inserter-throughput-toggle']
        if button then
            local new_state = settings.get_player_settings(player_index)[setting_name].value
...
local function on_toggle(event)
    ...
    local enabled_setting = player_settings['priyutils-inserter-throughput-enabled']
    enabled_setting.value = not enabled_setting.value
    -- this will trigger on_setting_changed above
    player_settings['priyutils-inserter-throughput-enabled'] = enabled_setting
end
...
local function on_gui_clicked(event)
    ...
    if event.element.name == 'priyutils-inserter-throughput-toggle' then
        on_toggle(event)
    end
end
...
local function stopwatch_command(event)
    ...
    if not event.player_index then
        ...
        localised_print{'priyutils-inserter-throughput.stopwatch-help'}
        return
    end
    ...
    if not inserter or inserter.type ~= 'inserter' or name_blacklist[inserter.name] then
        player.print{'priyutils-inserter-throughput.stopwatch-help'}
        return
    end
...
commands.add_command(
    'priyutils-it-stopwatch',
    {'', {'priyutils-inserter-throughput.stopwatch-command-description'}, ' ', {'priyutils-inserter-throughput.stopwatch-help'}},
    stopwatch_command)

local function stopwatch_results(player, entry)
    local result = stopwatch.get_throughput(entry)
    local precision = player.mod_settings['priyutils-inserter-throughput-rounding-precision'].value
    local pos = entry.entity_position
    local message = {'',
        {'priyutils-inserter-throughput.stopwatch-finished', {'priyutils-inserter-throughput.inserter-at',
            entry.entity_name, pos.x, pos.y, entry.entity_surface}},
        '\n',
    }
    local n = #message + 1
    if not result.stable then
        message[n] = {'priyutils-inserter-throughput.stopwatch-warning-not-max'}
        message[n + 1] = '\n'
        n = n + 2
    end
    message[n] = {'priyutils-inserter-throughput.stopwatch-result',
...
local function on_tick(event)
    ...
    for i = n, 1, -1 do
        ...
        elseif not inserter.valid then -- inserter is gone
            local pos = entry.entity_position
            player.print{'priyutils-inserter-throughput.stopwatch-interrupted', {'priyutils-inserter-throughput.inserter-at',
                entry.entity_name, pos.x, pos.y, entry.entity_surface}}
            end_watch = true
        elseif tick > entry.timeout_tick then
            local pos = entry.entity_position
            player.print{'priyutils-inserter-throughput.stopwatch-timeout', {'priyutils-inserter-throughput.inserter-at',
                entry.entity_name, pos.x, pos.y, entry.entity_surface}}
            end_watch = true
        else
...
function on_configuration_changed(event)
    ...
    local versions = event.mod_changes['PriyUtils']
...
script.on_event('priyutils-inserter-throughput-toggle', on_toggle)
