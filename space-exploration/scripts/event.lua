...
---@param event_key defines.events|string
---@param add_callback? fun(event:EventData)
...
Event.addListener = function(event_key, add_callback, virtual)
  if not Event.listeners[event_key] then
    ...
    local callbacks = Event.listeners[event_key].callbacks
...
---@param event_key defines.events|string
---@param remove_callback fun(event:EventData)
Event.removeListener = function(event_key, remove_callback)
  if not Event.listeners[event_key] then return end
  for i = #Event.listeners[event_key].callbacks, 1, -1 do -- Reverse iteration for safe removal
    local callback = Event.listeners[event_key].callbacks[i]
    if callback == remove_callback then
      table.remove(Event.listeners[event_key].callbacks, i)
    end
  end
end
...
---@param add_callback fun(event:EventData)
Event.addOnEntityCreatedListeners = function(add_callback)
  assert(add_callback)
  Event.addListener(defines.events.on_built_entity, add_callback)
  Event.addListener(defines.events.on_robot_built_entity, add_callback)
  Event.addListener(defines.events.on_space_platform_built_entity, add_callback)
  Event.addListener(defines.events.script_raised_built, add_callback)
  Event.addListener(defines.events.script_raised_revive, add_callback)
end

---@param add_callback fun(event:EventData)
Event.addOnEntityRemovedListeners = function(add_callback)
  assert(add_callback)
  Event.addListener(defines.events.on_player_mined_entity, add_callback)
  Event.addListener(defines.events.on_robot_mined_entity, add_callback)
  Event.addListener(defines.events.on_entity_died, add_callback)
  Event.addListener(defines.events.script_raised_destroy, add_callback)
  Event.addListener(defines.events.on_space_platform_mined_entity, add_callback)
end
