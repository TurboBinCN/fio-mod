...
local long_versions = {
  {pattern = "--t--", icon = "__space-exploration-graphics__/graphics/icons/naquium-heat-pipe-horizontal.png"},
  {pattern = "--t-----t--", icon = "__space-exploration-graphics__/graphics/icons/naquium-heat-pipe-horizontal-2.png"},
}

local vanilla_reactor = data.raw.reactor["nuclear-reactor"]
...
data:extend({
  {
    ...
    consumption = "400MW",
    neighbour_bonus = 1,
    circuit_connector = vanilla_reactor.circuit_connector,
    ...
    heat_buffer =
    {
      ...
      heat_glow =
      {
        filename = "__space-exploration-graphics-3__/graphics/entity/antimatter-reactor/reactor-heat-glow.png",
        priority = "extra-high",
        width = 188,
        height = 190,
...
local pipe_glow = {
  filename = "__space-exploration-graphics__/graphics/entity/heat-pipe/heated-glow.png",
  width = 55,
  height = 55,
  priority = "extra-high",
  tint = glow_tint,
...
for _, version in pairs(long_versions) do
  ...
  local display_name = string.gsub(version.pattern,"t","+")
  ...
  for i = 1, #version.pattern do
    ...
    if i == 1 and (c == "-" or c == "t") then
      table.insert(connections,{
        position = {0, j},
        direction = defines.direction.north
      })
    end
    if i == length and (c == "-" or c == "t") then
      table.insert(connections,{
        position = {0, j},
        direction = defines.direction.south
      })
  ...
  data:extend{
    ...
    },
    {
      ...
      energy_required = 10,
      ingredients = {
        {type = "item", name = data_util.mod_prefix .. "naquium-heat-pipe", amount = math.ceil(length/2)},
      },
    ...
    },
    {
      type = "boiler",
      name = name,
      localised_name = {"entity-name."..data_util.mod_prefix.."naquium-heat-pipe-long", display_name},
      localised_description = {"entity-description."..data_util.mod_prefix.."naquium-heat-pipe-long", display_name},
      burning_cooldown = 20,
      ...
      output_fluid_box = {
        ...
        pipe_connections = { --bugfix for "Boiler with no output pipe connections face north when flipped" as of Factorio 1.1.80
          {position = {0.3,0}, direction = defines.direction.east},
          {position = {-0.3,0}, direction = defines.direction.west}
        },
      },
      resistances = {
        {
      ...
      pictures = {
        east = {structure = {layers = {{layers = long_pipes_horizontal}}}},
        north = {structure = {layers = {{layers = long_pipes_vertical}}}},
        south = {structure = {layers = {{layers = long_pipes_vertical}}}},
        west = {structure = {layers = {{layers = long_pipes_horizontal}}}},
      },
